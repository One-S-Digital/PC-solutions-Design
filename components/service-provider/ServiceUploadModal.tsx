      ...
      <div className="w-full max-w-2xl bg-white shadow-xl rounded-lg overflow-hidden transform transition-all duration-300 scale-100 opacity-100">
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-4 max-h-[calc(80vh-120px)] overflow-y-auto">
            <div>
              <label htmlFor="title" className="block text-sm font-medium text-gray-700 mb-1">Service Title <span className="text-red-500 ml-0.5">*</span></label>
            ...
            <div>
              <label htmlFor="description" className="block text-sm font-medium text-gray-700 mb-1">Description <span className="text-red-500 ml-0.5">*</span></label>
            ...
            <div>
              <label htmlFor="availability" className="block text-sm font-medium text-gray-700 mb-1">Availability <span className="text-red-500 ml-0.5">*</span></label>
              <input type="text" name="availability" id="availability" value={formData.availability} onChange={handleInputChange} required className={STANDARD_INPUT_FIELD} placeholder="e.g., Mon-Fri, 9am-5pm; By appointment only" />
            </div>

            <div>
              <label htmlFor="priceInfo" className="block text-sm font-medium text-gray-700 mb-1">Price Information</label>
              <input type="text" name="priceInfo" id="priceInfo" value={formData.priceInfo} onChange={handleInputChange} className={STANDARD_INPUT_FIELD} placeholder="e.g., CHF 75/hour, From CHF 500, Free consultation" />
            </div>
            
            <div>
              <label htmlFor="tags" className="block text-sm font-medium text-gray-700 mb-1">Tags (comma-separated)</label>
              <input type="text" name="tags" id="tags" value={(formData.tags || []).join(', ')} onChange={handleInputChange} className={STANDARD_INPUT_FIELD} placeholder="e.g., Child Safe, Eco Friendly, Certified" />
            </div>
            ...
            <div className="pt-2">
              ...
              {formData.imageUrl && !file && <p className="mt-2 text-sm text-gray-500">Current image: <a href={formData.imageUrl} target="_blank" rel="noopener noreferrer" className="text-swiss-mint hover:underline">View</a></p>}
            </div>
          </div>
